    ...
    for index, row in enumerate(dataframe.itertuples(index=True, name='Pandas')):
        ...
        if bearish_occurence != None:
            ...
            while (True):
                ...
                if can_draw:
                    for i in range(length + 1):
                        actual_bearish_lines[prev_pivot + i] = bearish_prev_pivot + (
        ...
        if bullish_occurence != None:
            ...
            while (True):
                ...
                if can_draw:
                    for i in range(length + 1):
                        actual_bullish_lines[prev_pivot + i] = bullish_prev_pivot + (
    ...
    if high_iterator[index] == index:
        ...
        for i in range(current_index - 1, current_index - 6, -1):
            ...
            if np.isnan(prev_pivot):
                return
            if ((dataframe['pivot_highs'][current_pivot] < dataframe['pivot_highs'][prev_pivot] and indicator[
                current_pivot] > indicator[prev_pivot])
                    or (dataframe['pivot_highs'][current_pivot] > dataframe['pivot_highs'][prev_pivot] and indicator[
                        current_pivot] < indicator[prev_pivot])):
...
def bullish_divergence_finder(dataframe, indicator, low_iterator, index):
    if low_iterator[index] == index:
        ...
        for i in range(current_index - 1, current_index - 6, -1):
            ...
            if np.isnan(prev_pivot):
                return
            if ((dataframe['pivot_lows'][current_pivot] < dataframe['pivot_lows'][prev_pivot] and indicator[
                current_pivot] > indicator[prev_pivot])
                    or (dataframe['pivot_lows'][current_pivot] > dataframe['pivot_lows'][prev_pivot] and indicator[
                        current_pivot] < indicator[prev_pivot])):
                return (prev_pivot, current_pivot)
    return None
